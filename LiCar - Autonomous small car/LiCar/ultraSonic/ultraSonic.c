...
void ultraSonic_Init(DIO_AllPINS_TYPE trigg, DIO_AllPINS_TYPE echoo)
{
    trig_pin = trigg;
    echo_pin = echoo;
    DIO_AllPINS_TYPE pin = PB0 ;

    DIO_PORT_Init(trig_pin/pin);
    DIO_PORT_Init(echo_pin/pin);
...
uint8_t ultraSonic_ReadCM(void)
{
    ...
    readtime = TimerACapture_PulseWidth(TIMER_W0, echo_pin);   /* Reading the wave (ON) time */
